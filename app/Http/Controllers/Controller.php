...
if(isset($_POST['btn-register'])){
	...
	if ($genre == 'genero') {
		...
		header('Location: http://localhost/webmail2/index.php?error=3');
	}
	if($conf_pass == $password){
		...
		header('Location: http://localhost/webmail2/index.php?error=1');
	}
}
...
if(isset($_POST['btn-login'])){
	...
	if($correo_log == $correo && $pass_log == $password){
		...
		header('Location: http://localhost/webmail2/index.php?error=2');
	}
}
//Mensaje enviado
